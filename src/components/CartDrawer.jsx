...
export default function CartDrawer({ isOpen, onClose, cartItems, onUpdateQuantity, settings, onViewInvoice }) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    address: '',
    paymentMethod: 'Mobile Money (Moov, Mixx by Yas, Wave, Orange, MTN)'
  });
          ...
          {step === 'checkout' && (
            <form id="checkout-form" onSubmit={handleCheckoutSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <div className="form-group">
                ...
                >
                  <option value="Mobile Money (Moov, Mixx by Yas, Wave, Orange, MTN)">📱 Mobile Money (Moov Afrique, Mixx by Yas, Wave, Orange, MTN)</option>
              ...
              {formData.paymentMethod.includes('Mobile Money') && (
                <div className="glass-card" style={{ padding: '0.85rem', fontSize: '0.8rem', color: '#475569', background: '#f8fafc' }}>
                  ...
                  Effectuez le transfert vers le numéro <strong>{settings?.whatsappNumber || '22890000000'}</strong> (Moov Money / Mixx by Yas / Wave / Orange / MTN) avec la référence de commande qui vous sera attribuée.
                </div>
              )}
            </form>
          )}
